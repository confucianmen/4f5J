/**
 * Created by qhao on 2016/11/24.
 * author qhao
 * @date 2016/11/24
 */
var rechargeInfo = {
    networkType:"CMCC",
    package:"flexible-sharing",
    flow:10,
    time:1,
    number:2,
    totalFlow:20,
    ...
    U.slider = function(selection, options){
        ...
        _t.showValue = function(){
            if(_t.hasInput){
                ...
                //console.log(_t.currentValue);
                switch (_t.currentValue){
                    case 12:
                        rechargeInfo.discount = 0.85;
                        break;
                    case 24:
                        rechargeInfo.discount = 0.7;
                        break;
                    case 36:
                        rechargeInfo.discount = 0.5;
                        break;
                    default :
                        rechargeInfo.discount = 1;
                        break;
                };
                if(_t.currentValue >= 12){
                    $(".yearDescription").addClass("actived");
                    $(".yearDescription").css({
                        left:_t.currentValue/12*67+420 + "px"
                    });
                    $(".yearDescription .year-value").text(_t.currentValue/12);
                    $(".yearDescription .discount").text(rechargeInfo.discount.toString().split(".")[1].length > 1 ? rechargeInfo.discount*100 : rechargeInfo.discount*10);
                }
                else{
                    if($(".yearDescription").hasClass("actived")){
                        $(".yearDescription").removeClass("actived");
                    }
                }

                rechargeInfo.time = _t.currentValue;

                $(".recharge-choosen-detail .time").text(rechargeInfo.time < 12 ? rechargeInfo.time + "个月" : rechargeInfo.time/12 + "年");
                $(".recharge-choosen-detail .price").text("正在计算价格…");
            }
        };
        ...
        _t.setDomValue = function(value){
            console.log(_t.currentValue);
            rechargeInfo.time = _t.currentValue;

            rechargeInfo.price = rechargeInfo.time * rechargeInfo.flowPrice  * rechargeInfo.totalFlow * rechargeInfo.discount;

            $(".recharge-choosen-detail .time").text(rechargeInfo.time < 12 ? rechargeInfo.time + "个月" : rechargeInfo.time/12 + "年");
            $(".recharge-choosen-detail .price").text("￥" + rechargeInfo.price);
        };
